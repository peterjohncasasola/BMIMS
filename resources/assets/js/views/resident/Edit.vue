    ...
    getResidentById(id) {
      axios
        ...
        .then(response => {
          ...
          this.is_alreadyVoter = this.form.is_voter;
        })
        .catch(error => {
          console.log(error);
        });
    ...
    fetchPuroks() {
      axios.get("/api/getAllPuroks").then(res => {
        Object.keys(res.data).forEach(row => {
          this.purok_data.push(res.data[row].name);
        });
      });
    },
    updateResident() {
      this.form
        .put("/api/residents/" + this.form.id)
        .then(response => {
          console.log(response.data);
          this.voter_form.resident_id = response.data.id;
          this.$toastr.s("resident successfully updated");
        })
        .catch(err => {
          this.$toastr.e(this.voter_form.errors.errors.error);
        });

      if (this.is_alreadyVoter) {
        if (this.form.is_voter) {
          this.voter_form
            .put("/api/voters/" + this.voter_form.id)
            .then(response => {
              this.clearForm();
              this.clearErrors();
            })
      ...
      this.$route.push({ name: "resident-index" });
      this.$toastr.s("resident successfully updated");
    }
  },
  created() {
